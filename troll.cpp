...
inline float Species::GPPleaf(float PPFD, float VPD, float T) {
    ...
    if (s_calculated[convT]){
        ...
        tempRday += s_LookUp_tempRday[convT];
        VcmaxT = s_LookUp_VcmaxT[convT];
        JmaxT = s_LookUp_JmaxT[convT];

    }
    else {

        s_calculated[convT] = true;

        s_LookUp_tempRday[convT] = s_Rdark * LookUp_tempRday[convT];
        tempRday += s_LookUp_tempRday[convT];
        VcmaxT = s_LookUp_VcmaxT[convT] = s_Vcmax * LookUp_VcmaxT[convT];
        JmaxT = s_LookUp_JmaxT[convT] = s_Jmax * LookUp_JmaxT[convT];
        s_LookUp_thetaJmaxT[convT] = 4.0 * theta * JmaxT;
    ...
    float I = alpha * PPFD;
    float Ji = JmaxT + I;
    float J = (Ji - sqrt(Ji * Ji - s_LookUp_thetaJmaxT[convT] * I)) * halfDividedByTheta;

    return minf(VcmaxT / (s_fci + KmT), 0.25 * J / (s_fci + 2.0 * GammaT)) * (s_fci - GammaT);
